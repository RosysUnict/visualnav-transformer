...
def process_traj(filename, recon_dir, output_dir):
    ...
    traj_data = {"position": position_data, "yaw": yaw_data, "gps": gps}
...
def main(args: argparse.Namespace):
    ...
    for filename in tqdm.tqdm(filenames, desc="Trajectories processed"):
        p = mp.Process(target=process_traj, args=(filename, recon_dir, output_dir))
        processes.append(p)
        p.start()
